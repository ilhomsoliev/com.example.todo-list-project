...
import com.example.data.model.dto.LoginRequest
import com.example.data.model.dto.RegisterRequest
...
@OptIn(KtorExperimentalLocationsAPI::class)
@Location(USER_BY_EMAIL)
class UserByEmailRoute
...
fun Route.UserRoutes(
    ...
    get<UserByEmailRoute>{
        try {
            val email = call.request.queryParameters["email"]!!.toString()
            val user: User? = db.findUserByEmail(email = email)
            call.respond(HttpStatusCode.OK,SimpleResponse(success = true, user?.userName.toString()))
        } catch (e: Exception){
